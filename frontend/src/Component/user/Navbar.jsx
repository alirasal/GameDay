...
export default function Navbar() {
  ...
  useEffect(() => {
    // Fetch user information from your backend upon component mount
    fetch("http://localhost:8080/auth/login/success", {
      ...
      .then((data) => {
        if (data.error) {
          // Handle error or redirect to login if not authenticated
          navigate("/");
        } else {
          setUser(data.user);
            ...
            <div className="relative flex h-16 items-center justify-between">
              ...
              <div className="absolute inset-y-0 right-0 gap-3 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                {user ? (
                  ...
                  <button onClick={googleAuth} className="text-green-800">
                    Login
                  </button>
                )}
              </div>
